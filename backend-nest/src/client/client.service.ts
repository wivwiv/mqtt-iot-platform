...
import { ConnectResult, MQTTACLAccess, ACLResult, MQTTACLAlow } from "../support/enum";
...
export class ClientService {
    ...
    async checkACL(data: MQTTACLDto): Promise<ACLResult> {
        ...
        const aclList = await ACL.findAll({
            where,
            attributes: ['clientid', 'username', 'access', 'topic', 'allow', 'ipAddress', 'rank', 'version'],
            order: [['rank', 'DESC']],
        });
        ...
        aclResult = firstMatchedACL.access === MQTTACLAlow.ALLOW
            ? ACLResult.ALLOW
            : ACLResult.DENY;
        return aclResult;
    }
